    ...
    switch (keycode) {
        case A_UM:
            if (record->event.pressed) {
                a_timer = timer_read();   // start timer when pressed
            } else {
                if (timer_elapsed(a_timer) > 300) {
                    // Send ä (Alt+132 on Windows or Unicode input)
                    send_unicode_string("ä");
                } else {
                    tap_code(KC_A);  // Normal 'a'
                }
            }
...
[MAC_BASE] = LAYOUT_tkl_iso(
     ...
     KC_CAPS,  A_UM,     KC_S,     KC_D,     KC_F,     KC_G,     KC_H,     KC_J,     KC_K,     KC_L,     KC_SCLN,  KC_QUOT,  KC_NUHS,  KC_ENT,
